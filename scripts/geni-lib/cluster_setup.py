...
def _wait_controller_ready(ex: RemoteExecutor, timeout: int = 600) -> None:
    ...
    while time.time() - start < timeout:
        ok, out, _ = ex.exec("kubectl get pod -n kube-system -l component=kube-controller-manager -o jsonpath='{.items[0].status.conditions[?(@.type==\"Ready\")].status}' 2>/dev/null")
        if ok == 0 and out.strip("'\" ")=="True":
            print("controller manager pod is Ready!")
            return
        ok, _, _ = ex.exec("kubectl get secret -n kube-system -o json 2>/dev/null | jq -e '.items[] | select(.type==\"kubernetes.io/service-account-token\")' >/dev/null 2>&1")
        if ok == 0:
